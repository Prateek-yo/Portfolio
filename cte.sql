...
SELECT e.employee_name, SUM(s.sale_amount) AS totalSalesAmount
FROM employees e
JOIN sales s ON e.employee_id = s.employee_id
GROUP BY e.employee_name
 HAVING totalSalesAmount > 10000;

SELECT 
    e.employee_name,
    SUM(s.sale_amount) AS totalSalesAmount,
    CASE 
        WHEN SUM(s.sale_amount) > 10000 THEN 'jhakhas'
        ELSE 'lag gaye'
...
HAVING 
    total_spent > (
        ...
        FROM (
            SELECT SUM(s2.sale_amount) AS total_spent
            FROM sales s2
